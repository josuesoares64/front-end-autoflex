...
export default function MaterialModal({ isOpen, onClose, selectedMaterial }: Props) {
  ...
  useEffect(() => {
    if (selectedMaterial) {
      setFormData({
        ...
        stockQuantity: String(selectedMaterial.stockQuantity)
      });
    } else {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1">Material Code</label>
            <input 
              required
              value={formData.code}
              className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-black transition"
